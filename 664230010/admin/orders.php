...
$stmt = $conn->query("SELECT o.*, u.username FROM orders o LEFT JOIN users u ON o.user_id = u.user_id ORDER BY o.order_date DESC ");
$orders = $stmt->fetchAll(PDO::FETCH_ASSOC);

require '../function.php'; // ฟังก์ชันที่เก็บไว้
...
<h2 class="text-center mb-4">คำสั่งซื้อทั้งหมด</h2>
    ...
    <div class="accordion-item">
        <h2 class="accordion-header" id="heading<?= $index ?>">
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?= $index ?>" aria-expanded="false" aria-controls="collapse<?= $index ?>">
                คำสั่งซื้อ #<?= $order['order_id'] ?> | <?= htmlspecialchars($order['username']) ?> | <?= $order['order_date'] ?> | สถานะ: <span class="badge bg-info"><?= ucfirst($order['status']) ?></span>
            </button>
        </h2>
